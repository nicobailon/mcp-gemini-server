...
describe("fileUtils", () => {
  ...
  describe("Functional Tests", () => {
    ...
    it("should throw error when filePath resolves outside allowed paths", async () => {
      ...
      // Act & Assert
      await expect(secureWriteFile(unsafePath, TEST_CONTENT, allowedPaths))
    ...
    it("should throw error when filePath uses directory traversal to escape allowed path", async () => {
      // Arrange
      const traversalPath = path.join(ALLOWED_DIR, "subdir", "..", "..", "outside", "file.txt");
      const allowedPaths = [ALLOWED_DIR];
      
      // Act & Assert
      await expect(secureWriteFile(traversalPath, TEST_CONTENT, allowedPaths))
        .rejects.toThrow(/not within the allowed output locations/);
    });
    ...
    it("should throw error when no allowed paths are provided", async () => {
      ...
      // Act & Assert
      await expect(secureWriteFile(filePath, TEST_CONTENT, allowedPaths))
        .rejects.toThrow(/not within the allowed output locations/);
    });

    it("should correctly handle path normalization and resolution", async () => {
      // Arrange
      const complexPath = path.join(ALLOWED_DIR, ".", "subdir", "..", "normalized-file.txt");
      const allowedPaths = [ALLOWED_DIR];
