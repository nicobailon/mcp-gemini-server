...
import { GeminiApiError, GeminiModelError, GeminiErrorMessages } from "../../../../src/utils/geminiErrors.js";
...
describe("GeminiService - Image Generation", () => {
  ...
  beforeEach(() => {
    ...
    GeminiService.prototype.generateImage = async function(
      prompt, modelName, resolution, numberOfImages, safetySettings, negativePrompt, stylePreset, seed, styleStrength
    ) {
      try {
        ...
        const generationConfig: Record<string, string | number | object | boolean | undefined> = {
        ...
        if (styleStrength !== undefined) generationConfig.styleStrength = styleStrength;
  ...
  afterEach(() => {
    ...
    if ((GeminiService as any)._originalGenerateImage) {
      GeminiService.prototype.generateImage = (GeminiService as any)._originalGenerateImage;
  ...
  it("should generate an image with custom parameters", async () => {
    ...
    const safetySettings = [{ category: "HARM_CATEGORY_HARASSMENT", threshold: "BLOCK_MEDIUM_AND_ABOVE" }] as any;
  ...
  it("should throw an error when no images are generated", async () => {
    ...
    await assert.rejects(
      async () => await service.generateImage(prompt),
      (err) => {
        return err instanceof GeminiModelError && 
  ...
  it("should support advanced style parameters", async () => {
    ...
    // Act
    await service.generateImage(prompt, undefined, undefined, 1, undefined, undefined, stylePreset, seed, styleStrength);
