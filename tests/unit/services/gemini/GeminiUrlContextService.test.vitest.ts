...
import { GeminiUrlFetchError, GeminiUrlValidationError } from '../../../../src/utils/geminiErrors.js';
...
describe('GeminiUrlContextService', () => {
  ...
  beforeEach(() => {
    ...
    service = new GeminiUrlContextService(mockConfig as ConfigurationManager, mockLogger as Logger);
  });
  ...
  describe('fetchUrlContent', () => {
    ...
    it('should handle fetch errors gracefully', async () => {
      ...
      await expect(service.fetchUrlContent('https://example.com/error')).rejects.toThrow(GeminiUrlFetchError);
    });
    ...
    it('should handle HTTP error responses', async () => {
      ...
      await expect(service.fetchUrlContent('https://example.com/notfound')).rejects.toThrow(GeminiUrlFetchError);
    });
    ...
    it('should respect content size limits', async () => {
      ...
      const result = await service.fetchUrlContent('https://example.com/large', {
    ...
    it('should handle JSON content without conversion', async () => {
      const jsonContent = JSON.stringify({ message: 'Hello World', data: [1, 2, 3] });
      ...
      const result = await service.fetchUrlContent('https://api.example.com/data', {
  ...
  describe('processUrlsForContext', () => {
    it('should process multiple URLs successfully', async () => {
      ...
      const mockContent1 = '<html><head><title>Page 1</title></head><body><p>Content 1</p></body></html>';
      const mockContent2 = '<html><head><title>Page 2</title></head><body><p>Content 2</p></body></html>';
      ...
      expect(result.contents[0].parts[0].text).toContain('Content from https://example1.com');
      expect(result.contents[1].parts[0].text).toContain('Content from https://example2.com');
    });
  ...
  describe('Content metadata extraction', () => {
    ...
    it('should handle HTML entities in metadata', async () => {
      ...
      expect(result.metadata.description).toBe('Description with "quotes" and   spaces');
    });
  });
  ...
  describe('Rate limiting', () => {
    it('should enforce rate limits per domain', async () => {
      ...
      await expect(service.fetchUrlContent(`${url}?page=11`)).rejects.toThrow(GeminiUrlFetchError);
    });
  });
  ...
  describe('Error handling', () => {
    it('should handle timeout errors', async () => {
      ...
      await expect(service.fetchUrlContent('https://example.com/timeout')).rejects.toThrow(GeminiUrlFetchError);
    });
    ...
    it('should handle unsupported content types', async () => {
      ...
      await expect(service.fetchUrlContent('https://example.com/binary')).rejects.toThrow(GeminiUrlFetchError);
    });
  });
});