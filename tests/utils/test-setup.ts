 ...
 */
export async function setupTestServer(options: TestServerOptions = {}): Promise<TestServerContext> {
  ...
  return {
    ...
    teardown: async () => {
      ...
      process.env.GEMINI_SAFE_FILE_BASE_DIR = originalEnv.GEMINI_SAFE_FILE_BASE_DIR;
 ...
 */
export function checkRequiredEnvVars(requiredVars: string[] = ['GOOGLE_GEMINI_API_KEY']): boolean {
  const missing = requiredVars.filter(varName => !process.env[varName]);
  if (missing.length > 0) {
    console.warn(`Missing required environment variables for testing: ${missing.join(', ')}`);
    console.warn('Create a .env.test file or set these variables in your environment');
    return false;
  }
  return true;
}
 ...
 */
export function skipIfMissingEnvVars(t: any, requiredVars: string[] = ['GOOGLE_GEMINI_API_KEY']): boolean {
  const missing = requiredVars.filter(varName => !process.env[varName]);
  if (missing.length > 0) {
