...
export const CreateCacheParamsObject = {
  model: z
    ...
    .describe(
      "Optional. The name/ID of the model compatible with caching (e.g., 'gemini-1.5-flash'). If omitted, the server's default model (from GOOGLE_GEMINI_MODEL env var) will be used."
    ),
  ...
  displayName: z
    ...
    .max(100)
    .optional()
    .describe("Optional. A human-readable name for the cache."),
  systemInstruction: ContentSchema.optional().describe(
    // Assuming ContentSchema can represent a single Content object too
