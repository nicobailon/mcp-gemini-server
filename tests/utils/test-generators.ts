 ...
 */
export function generateTestPrompt(options: PromptGeneratorOptions = {}): string {
  const {
    minLength = 10,
    maxLength = 100,
  ...
  const starter = starters[topic][Math.floor(Math.random() * starters[topic].length)];
  const subject = subjects[topic][Math.floor(Math.random() * subjects[topic].length)];
  const followUp = followUps[topic][Math.floor(Math.random() * followUps[topic].length)];
 ...
 */
export function generateTestContentArray(messageCount: number = 3, includeImages: boolean = false): any[] {
  const contents = [];
 ...
 */
export function generateFunctionCall(functionName: string, args: Record<string, any> = {}): any {
  return {
    name: functionName,
    args
  };
...
export function generateBoundingBoxes(objectCount: number = 3): any[] {
  const commonObjects = [
    'dog', 'cat', 'person', 'car', 'chair', 'table', 'book', 
    'bottle', 'cup', 'laptop', 'phone', 'plant', 'bird'
  ];
 ...
 */
export function generateBase64Image(type: 'pixel' | 'gradient' | 'checkerboard' = 'pixel'): string {
  ...
  const images = {
    ...
    // 2x2 gradient PNG
    gradient: 'iVBORw0KGgoAAAANSUhEUgAAAAIAAAACCAYAAABytg0kAAAAFElEQVQIW2P8z8Dwn4EIwMDAwMAAACbKAxI3gV+CAAAAAElFTkSuQmCC',
    // 4x4 checkerboard PNG
    checkerboard: 'iVBORw0KGgoAAAANSUhEUgAAAAQAAAAECAYAAACp8Z5+AAAAG0lEQVQIW2NkYGD4z8DAwMgABUwM0QCQHiYGAFULAgVoHvmSAAAAAElFTkSuQmCC'
  };
