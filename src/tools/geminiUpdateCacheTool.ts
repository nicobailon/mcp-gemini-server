 ...
 */
export const geminiUpdateCacheTool = (server: McpServer, geminiService: GeminiService): void => {

    // Define the tool processing function
    const processRequest = async (params: UpdateCacheParams): Promise<{ content: { type: 'text', text: string }[] }> => {
        logger.info(`Processing ${TOOL_NAME_UPDATE_CACHE} request for cache: ${params.cacheName}`);
        ...
        try {
            ...
            const cacheMetadata: CachedContentMetadata = await geminiService.updateCache(
                params.cacheName,
                updates
            );

            logger.info(`Successfully updated metadata for cache: ${cacheMetadata.name}`);
            ...
            logger.error(`Error processing ${TOOL_NAME_UPDATE_CACHE} for cache ${params.cacheName}:`, error);
            ...
            if (error instanceof GeminiApiError) {
                ...
                if (error.message.toLowerCase().includes('invalid ttl')) {
                    throw new McpError(ErrorCode.InvalidParams, `Invalid TTL format provided: ${params.ttl}`, error.details);
                }
                ...
                throw new McpError(ErrorCode.InternalError, 'An unknown error occurred while updating cache metadata.');
            }
        }
    };
