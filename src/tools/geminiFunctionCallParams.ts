...
const functionParameterTypeSchema = z
  .enum([
    ...
    "BOOLEAN",
    "ARRAY",
    "INTEGER", // Based on SDK's Type enum (approximated)
  ])
...
export const functionDeclarationSchema = z
  .object({
    name: z
      ...
      .min(1)
      .describe(
        "The name of the function to be called. Must match the name the model is expected to use."
      ),
    description: z
      .string()
      .min(1)
    ...
    parameters: z
      ...
      .describe(
        "Schema defining the parameters the function accepts. Must be of type OBJECT."
      ),
  })
  .describe(
...
export const GEMINI_FUNCTION_CALL_PARAMS = {
  ...
  functionDeclarations: z
    ...
    .min(1)
    .describe(
      "Required. An array of function declarations (schemas) that the model can choose to call based on the prompt."
    ),
