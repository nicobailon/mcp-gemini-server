...
describe("MCP Client Tools Unit Tests", () => {
  ...
  let mockSecureWriteFile: any;
  
  // Function to create and capture the processor function
  const captureProcessorFunction = (mock: any): Function => {
  ...
  beforeEach(() => {
    ...
    // Import and mock the secureWriteFile function
    mockSecureWriteFile = mock.fn().mockResolvedValue(undefined);
    ...
    global.jest = {
      ...
      mocks: {
        ...
        "../../../src/utils/fileUtils.js": {
          secureWriteFile: mockSecureWriteFile,
        },
      },
    };
  });
  ...
  describe("mcpCallServerTool", () => {
    ...
    it("should throw error when no allowed output paths are configured", async () => {
      ...
      await assert.rejects(
        ...
        (error: any) => {
          assert.ok(error instanceof McpError);
          assert.strictEqual(error.code, ErrorCode.InvalidRequest);
