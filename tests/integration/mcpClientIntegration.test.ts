...
type ToolProcessor = (...args: any[]) => Promise<any>;
...
function restoreConfigurationManager(): void {
  // @ts-ignore - Restore the original getInstance method
  delete ConfigurationManager.getInstance;
}

// Generic function to capture tool processor from tool registration
function captureToolProcessor(toolFn: Function, mcpClientService: McpClientService): ToolProcessor {
  // Create a mock MCP server
  const mockMcpServer = {
    tool: (_name: string, _description: string, _schema: any, processor: ToolProcessor) => {
      return processor;
    }
...
async function startDummyMcpServerStdio(): Promise<ChildProcess> {
  ...
  const nodeProcess = spawn('node', [serverPath], {
    ...
    env: { ...process.env, NODE_OPTIONS: '--no-warnings --experimental-specifier-resolution=node' },
  });
  ...
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      nodeProcess.kill();
      reject(new Error(`Timeout waiting for dummy server to start. Last output: ${errorOutput}`));
    }, 5000);
...
describe('MCP Client Integration Tests', () => {
  ...
  describe('STDIO Transport Tests', () => {
    ...
    it('should connect to a stdio server, list tools, call a tool, and disconnect', async () => {
      ...
      assert.strictEqual(typeof connectionId, 'string', 'Connection ID should be a string');
      ...
      const toolNames = toolsList.map((tool: any) => tool.name);
      assert.ok(toolNames.includes('echoTool'), 'Echo tool should be available');
      assert.ok(toolNames.includes('addTool'), 'Add tool should be available');
      assert.ok(toolNames.includes('complexDataTool'), 'Complex data tool should be available');
      ...
      // Verify echo result
      assert.strictEqual(echoResult.message, echoMessage, 'Echo tool should return the sent message');
      ...
      // Verify add result
      assert.strictEqual(addOutput.sum, 12, 'Add tool should correctly sum the numbers');
      assert.deepStrictEqual(addOutput.inputs, { a: 5, b: 7 }, 'Add tool should return the input values');
      ...
      assert.strictEqual(disconnectOutput.connectionId, connectionId, 'Disconnect should return the connection ID');
      assert.ok(disconnectOutput.message.includes('Connection closed'), 'Disconnect should confirm connection closure');
      
      // Verify the connection is no longer in the active connections list
      assert.strictEqual(mcpClientService.getActiveStdioConnectionIds().length, 0, 'No active stdio connections should remain');
    });
    ...
    it('should call a tool and write output to a file', async () => {
      ...
      assert.strictEqual(callToolOutput.message, 'Output written to file', 'Should confirm file was written');
      assert.strictEqual(callToolOutput.filePath, outputPath, 'Should return the correct file path');
      ...
      assert.strictEqual(fileData.items.length, 3, 'Should have 3 items as specified');
      assert.strictEqual(fileData.items[0].level, 2, 'Nested level should be 2');
  ...
  describe('SSE Transport Tests', () => {
    ...
    it('should connect to an SSE server, list tools, call a tool, and disconnect', async () => {
      ...
      // Verify the connection is no longer in the active connections list
      assert.strictEqual(mcpClientService.getActiveSseConnectionIds().length, 0, 'No active SSE connections should remain');
    });
  });
  ...
  describe('Write to File Tool Tests', () => {
    ...
    beforeEach(() => {
      ...
      mockMcpServer = {
        tool: (_name: string, _desc: string, _schema: any, processor: ToolProcessor) => processor
      } as unknown as McpServer;
      
      // Register the writeToFileTool and capture its processor
      writeToFileProcessor = writeToFileTool(mockMcpServer) as unknown as ToolProcessor;
    });
    ...
    it('should write a string to a file', async () => {
      ...
      const testContent = 'This is a test string to write to a file\nWith multiple lines\nAnd special chars: €£¥©®™';
      ...
      // Verify the result contains the expected information
      assert.strictEqual(resultJson.message, 'Content written to file successfully.', 'Should confirm file was written');
      assert.strictEqual(resultJson.filePath, testFilePath, 'Should return the correct file path');
      ...
      assert.strictEqual(fileContent, testContent, 'File content should match the original string');
    });
    ...
    it('should write a base64 encoded string to a file', async () => {
      ...
      const originalString = 'This is a test string that will be base64 encoded\nWith multiple lines\nAnd special chars: €£¥©®™';
      ...
      // Read the file and compare the content
      const fileContent = await fs.readFile(testFilePath, 'utf8');
      assert.strictEqual(fileContent, originalString, 'File content should match the original string after base64 decoding');
    });
    ...
    it('should fail when writing to a path outside allowed directories', async () => {
      ...
      const nonAllowedPath = path.join(os.tmpdir(), '..', 'non-allowed-dir', 'test.txt');
      ...
      await assert.rejects(
        ...
        (error: any) => {
          assert.ok(error.message.includes('Security error') || 
                    ...
                    error.message.includes('InvalidParams'), 
      ...
      const fileExists = await fs.access(nonAllowedPath).then(() => true).catch(() => false);
      assert.strictEqual(fileExists, false, 'File should not have been created');
    });
  });
});