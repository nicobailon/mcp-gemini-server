...
describe('URL Context Integration Tests', () => {
  ...
  beforeEach(() => {
    ...
    vi.mocked(await import('@google/genai')).GoogleGenAI = vi.fn().mockImplementation(() => mockGenAI);
  ...
  describe('URL Context with Content Generation', () => {
    ...
    it('should handle multiple URLs in context', async () => {
      ...
      const result = await geminiService.generateContent({
        prompt: 'Compare the development approaches mentioned in these articles',
        urlContext: {
          urls: [
            'https://example1.com/react',
            'https://example2.com/vue'
          ]
        }
      });
  ...
  describe('URL Context Error Handling', () => {
    ...
    it('should handle mixed success/failure scenarios', async () => {
      const mockSuccessContent = '<html><body><p>Successful content</p></body></html>';
  ...
  describe('URL Security Integration', () => {
    ...
    it('should enforce URL count limits', async () => {
      const manyUrls = Array.from({ length: 25 }, (_, i) => `https://example${i}.com`);
