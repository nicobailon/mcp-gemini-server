...
export function assertValidContentResponse(response: any): void {
  ...
  assert.ok(response.candidates.length > 0, 'Candidates array should not be empty');
  ...
  const hasSomeText = candidate.content.parts.some((part: any) => 
 ...
 */
export function assertValidImageResponse(response: any, expectedCount: number = 1): void {
  assert.ok(response, 'Response should not be null or undefined');
  ...
  assert.strictEqual(response.images.length, expectedCount, 
  ...
  for (let i = 0; i < response.images.length; i++) {
    ...
    assert.ok(typeof image.base64Data === 'string', `Image ${i} base64Data should be a string`);
    assert.ok(image.base64Data.length > 0, `Image ${i} base64Data should not be empty`);
    
    assert.ok(image.mimeType, `Image ${i} should have mimeType`);
    assert.ok(typeof image.mimeType === 'string', `Image ${i} mimeType should be a string`);
    ...
    assert.ok(typeof image.width === 'number', `Image ${i} width should be a number`);
    assert.ok(image.width > 0, `Image ${i} width should be positive`);
    
    assert.ok(image.height, `Image ${i} should have height`);
    assert.ok(typeof image.height === 'number', `Image ${i} height should be a number`);
...
export function assertMcpError(
  ...
  assert.strictEqual(error.code, expectedCode, `Error code should be ${expectedCode}`);
...
export function assertValidBoundingBoxes(objects: any[]): void {
  ...
  for (let i = 0; i < objects.length; i++) {
    ...
    assert.ok(typeof obj.label === 'string', `Object ${i} label should be a string`);
    ...
    assert.ok(box.xMin >= 0 && box.xMin <= 1000, `Box ${i} xMin should be between 0 and 1000`);
    
    assert.ok(typeof box.yMin === 'number', `Box ${i} yMin should be a number`);
    assert.ok(box.yMin >= 0 && box.yMin <= 1000, `Box ${i} yMin should be between 0 and 1000`);
    
    assert.ok(typeof box.xMax === 'number', `Box ${i} xMax should be a number`);
    assert.ok(box.xMax >= 0 && box.xMax <= 1000, `Box ${i} xMax should be between 0 and 1000`);
    
    assert.ok(typeof box.yMax === 'number', `Box ${i} yMax should be a number`);
    assert.ok(box.yMax >= 0 && box.yMax <= 1000, `Box ${i} yMax should be between 0 and 1000`);
...
export function assertValidSessionId(sessionId: string): void {
  ...
  assert.ok(validIdPattern.test(sessionId), 'Session ID should have a valid format');
}
...
export function assertValidCacheId(cacheId: string): void {
  ...
  assert.ok(cacheId.startsWith('cachedContents/'), 'Cache ID should start with "cachedContents/"');
}
