...
export const TOOL_DESCRIPTION_AUDIO_TRANSCRIPTION =
  "Transcribes audio files using Gemini 2.5 models. Supports various audio formats and can include timestamps, language specification, and custom prompts. Handles both direct processing (files <20MB) and File API processing (files up to 2GB with Google AI Studio API key).";
...
export const AudioTranscriptionParamsObject = {
  filePath: z.string({
    description:
      "Absolute path to the audio file. Must be accessible by the server process.",
  ...
  modelName: z
    .string({
      description:
        "Compatible Gemini 2.5 model to use for transcription. Example: 'gemini-2.5-pro'",
    })
    .optional(),
  includeTimestamps: z
    .boolean({
  ...
  language: z
    .string({
      description:
        "BCP-47 language code (e.g., 'en-US', 'fr-FR') to optimize transcription for a specific language",
    })
    .optional(),
  prompt: z
    .string({
  ...
  mimeType: z
    .enum(SUPPORTED_AUDIO_FORMATS, {
      description:
        "Audio file MIME type. Recommended to provide for accurate processing.",
    })
    .optional(),
};
