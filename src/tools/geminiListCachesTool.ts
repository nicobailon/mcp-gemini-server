 ...
 */
export const geminiListCachesTool = (server: McpServer, geminiService: GeminiService): void => {

    // Define the tool processing function
    const processRequest = async (params: ListCachesParams): Promise<{ content: { type: 'text', text: string }[] }> => {
        logger.info(`Processing ${TOOL_NAME_LIST_CACHES} request...`);
        logger.debug("Received params:", params);

        try {
            // Call the GeminiService method
            const result: { caches: CachedContentMetadata[]; nextPageToken?: string } = await geminiService.listCaches(
                params.pageSize,
                params.pageToken
            );
            ...
            if (result.nextPageToken) {
                ...
                logger.debug(`No next page token returned (iteration method used or last page).`);
            }
            ...
            if (error instanceof GeminiApiError) {
                ...
                throw new McpError(ErrorCode.InternalError, 'An unknown error occurred while listing caches.');
            }
        }
    };
