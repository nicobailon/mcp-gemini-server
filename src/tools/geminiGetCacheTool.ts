 ...
 */
export const geminiGetCacheTool = (server: McpServer, geminiService: GeminiService): void => {

    // Define the tool processing function
    const processRequest = async (params: GetCacheParams): Promise<{ content: { type: 'text', text: string }[] }> => {
        logger.info(`Processing ${TOOL_NAME_GET_CACHE} request for cache: ${params.cacheName}`);
        logger.debug("Received params:", params);

        try {
            // Call the GeminiService method
            const cacheMetadata: CachedContentMetadata = await geminiService.getCache(params.cacheName);

            logger.info(`Successfully retrieved metadata for cache: ${cacheMetadata.name}`);
            ...
            logger.error(`Error processing ${TOOL_NAME_GET_CACHE} for cache ${params.cacheName}:`, error);
            ...
            if (error instanceof GeminiApiError) {
                ...
                throw new McpError(ErrorCode.InternalError, 'An unknown error occurred while getting cache metadata.');
            }
        }
    };
