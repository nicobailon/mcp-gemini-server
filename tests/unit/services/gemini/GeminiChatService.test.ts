...
const mockGenerateContent = mock.fn<(config: any) => Promise<PartialGenerateContentResponse>>();
...
describe("GeminiChatService", () => {
  ...
  describe("startChatSession", () => {
    ...
    it("should create a new chat session with provided model", () => {
      ...
      const sessionId = chatService.startChatSession({ modelName: customModel });
  ...
  describe("sendMessageToSession", () => {
    ...
    it("should send a message to an existing session", async () => {
      ...
      mockGenerateContent.mock.mockImplementationOnce(() => Promise.resolve(mockResponse));
      ...
      const lastContent = requestConfig.contents[requestConfig.contents.length - 1];
    ...
    it("should apply per-message configuration options", async () => {
      ...
      mockGenerateContent.mock.mockImplementationOnce(() => Promise.resolve(emptyResponse));
  ...
  describe("routeMessage", () => {
    ...
    it("should pass system instruction to both routing and content requests", async () => {
      ...
      assert.strictEqual(routingConfig.contents[0].parts[0].text, systemInstruction);
      
      // Verify system instruction was added to content request
      const [messageConfig] = mockGenerateContent.mock.calls[1].arguments;
      assert.strictEqual(messageConfig.contents[0].role, "system");
      assert.strictEqual(messageConfig.contents[0].parts[0].text, systemInstruction);
    });
  });
});