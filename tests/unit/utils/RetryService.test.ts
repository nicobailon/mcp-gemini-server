...
describe("RetryService", () => {
  ...
  describe("execute method", () => {
    ...
    let onRetryMock: ReturnType<
      typeof mock.fn<(error: unknown, attempt: number, delayMs: number) => void>
    >;
    ...
    it("should use custom retryable error check if provided", async () => {
      ...
      await assert.rejects(
        async () => await customRetryService.execute(nonRetryableFn)
      );
      assert.strictEqual(nonRetryableFn.mock.callCount(), 1);

      // Should retry for custom error
      await assert.rejects(
        async () => await customRetryService.execute(retryableFn)
      );
