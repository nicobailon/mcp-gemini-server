    ...
    const response = await fetch(`${serverContext.baseUrl}/v1/tools`, {
      ...
      body: JSON.stringify({
        ...
        input: {
          ...
          routingPrompt: "Choose the best model for this question: factual knowledge or creative content?",
        },
      }),
    });
  ...
  it("should return validation errors for invalid inputs", async (t) => {
    ...
    serverContext.server.on('request', (req, res) => {
      if (req.url === '/v1/tools' && req.method === 'POST') {
        ...
        res.end(JSON.stringify({
          code: "InvalidParams",
          message: "Invalid parameters: message cannot be empty, models array cannot be empty",
          status: 400
        }));
        return;
      }
