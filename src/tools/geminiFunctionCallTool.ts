...
type GeminiFunctionCallArgs = z.infer<z.ZodObject<typeof GEMINI_FUNCTION_CALL_PARAMS>>;
 ...
 */
export const geminiFunctionCallTool = (server: McpServer, serviceInstance: GeminiService): void => {
    ...
    const processRequest = async (args: GeminiFunctionCallArgs) => {
        logger.debug(`Received ${GEMINI_FUNCTION_CALL_TOOL_NAME} request:`, { model: args.modelName });
        try {
            const {
                modelName,
                prompt,
            ...
            if (result && result.functionCall && typeof result.functionCall === 'object') {
                // It's a function call request
                logger.debug(`Function call requested by model ${modelName}: ${result.functionCall.name}`);
                ...
                logger.debug(`Text response received from function call request for model ${modelName}.`);
                return {
                    content: [{
                        type: "text" as const,
                        text: result.text
                ...
                logger.error(`Unexpected response structure from generateFunctionCallRequest for model ${modelName}:`, result);
                throw new Error('Invalid response structure received from Gemini service.');
            }

        } catch (error: unknown) {
            logger.error(`Error processing ${GEMINI_FUNCTION_CALL_TOOL_NAME}:`, error);
