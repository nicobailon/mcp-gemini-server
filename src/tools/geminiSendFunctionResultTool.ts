...
import type { GenerationConfig, SafetySetting, GenerateContentResponse, FunctionCall } from '@google/genai';
 ...
 */
export const geminiSendFunctionResultTool = (server: McpServer, serviceInstance: GeminiService): void => {
     ...
     */
    const processRequest = async (args: GeminiSendFunctionResultArgs): Promise<CallToolResult> => {
        logger.debug(`Received ${GEMINI_SEND_FUNCTION_RESULT_TOOL_NAME} request for session: ${args.sessionId}`);
        try {
            const { sessionId, functionResponses, generationConfig, safetySettings } = args;

            // Call the service to send the function results
            const response: GenerateContentResponse = await serviceInstance.sendFunctionResultToSession(
                sessionId,
            ...
            if (response.promptFeedback?.blockReason === BlockedReason.SAFETY) {
                logger.warn(`Gemini prompt blocked due to SAFETY for session ${sessionId} after function result.`);
                return {
                    content: [{
                        type: "text",
            ...
            if (firstCandidate?.finishReason && firstCandidate.finishReason !== FinishReason.STOP && firstCandidate.finishReason !== FinishReason.MAX_TOKENS) {
                if (firstCandidate.finishReason === FinishReason.SAFETY) {
                    logger.warn(`Gemini response stopped due to SAFETY for session ${sessionId} after function result.`);
                    return {
                        content: [{
                            type: "text",
                ...
                logger.warn(`Gemini response finished with reason ${firstCandidate.finishReason} for session ${sessionId} after function result.`);
            }
            ...
            if (subsequentFunctionCalls && subsequentFunctionCalls.length > 0) {
                logger.debug(`Returning subsequent function call(s) for session ${sessionId}: ${subsequentFunctionCalls.map(fc => fc.name).join(', ')}`);
                return {
                    content: [{
                        type: "text",
                ...
                logger.debug(`Returning text response for session ${sessionId} after function result.`);
                return {
                    content: [{
                        type: "text",
                        text: responseText
                ...
                logger.warn(`No text or subsequent function call found in Gemini response for session ${sessionId} after function result, finishReason: ${firstCandidate?.finishReason}. Returning empty content.`);
                return { content: [{ type: "text", text: "" }] };
            }
            // --- End Response Processing ---

        } catch (error: unknown) {
            logger.error(`Error processing ${GEMINI_SEND_FUNCTION_RESULT_TOOL_NAME} for session ${args.sessionId}:`, error);
            ...
            const errorMessage = error instanceof Error ? error.message : 'An unexpected error occurred sending function results.';
            throw new McpError(
