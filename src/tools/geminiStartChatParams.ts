...
const functionDeclarationSchema = z
  .object({
...
export const GEMINI_START_CHAT_PARAMS = {
  modelName: z
    ...
    .describe(
      "Optional. The name of the Gemini model to use for this chat session (e.g., 'gemini-1.5-flash'). If omitted, the server's default model (from GOOGLE_GEMINI_MODEL env var) will be used."
    ),
  history: z
    .array(HistoryContentSchema)
    .optional()
    .describe(
      "Optional. An array of initial conversation turns to seed the chat session. Must alternate between 'user' and 'model' roles, starting with 'user'."
    ),
  tools: z
    .array(ToolSchema)
    .optional()
    .describe(
      "Optional. A list of tools (currently only supporting function declarations) the model may use during the chat session."
    ),
  generationConfig: GenerationConfigSchema.optional().describe(
    "Optional. Session-wide generation configuration settings."
  ),
  safetySettings: z
  ...
  cachedContentName: z
    ...
    .optional()
    .describe(
      "Optional. Identifier for cached content in format 'cachedContents/...' to use with this chat session."
    ),
...
export type GeminiStartChatArgs = z.infer<
  z.ZodObject<typeof GEMINI_START_CHAT_PARAMS>
>;
