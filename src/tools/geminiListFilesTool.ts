 ...
 */
export const geminiListFilesTool = (server: McpServer, geminiService: GeminiService): void => {

    // Define the tool processing function
    const processRequest = async (params: ListFilesParams): Promise<{ content: { type: 'text', text: string }[] }> => {
        logger.info(`Processing ${TOOL_NAME_LIST_FILES} request...`);
        logger.debug("Received params:", params);

        try {
            // Call the GeminiService method
            const result: { files: FileMetadata[]; nextPageToken?: string } = await geminiService.listFiles(
                params.pageSize,
                params.pageToken
            );
            ...
            if (error instanceof GeminiApiError) {
                ...
                throw new McpError(ErrorCode.InternalError, 'An unknown error occurred while listing files.');
            }
        }
    };
