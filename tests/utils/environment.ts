...
export async function loadTestEnv(): Promise<void> {
  ...
  if (existsSync(envPath)) {
    try {
      ...
      console.warn(`Failed to load .env.test file: ${(error as Error).message}`);
    }
  } else {
    console.warn('.env.test file not found, using existing environment variables');
  }
}
...
`;
  ...
  try {
    ...
    console.error(`Failed to create .env.test.example file: ${(error as Error).message}`);
  }
}
 ...
 */
export function verifyEnvVars(requiredVars: string[] = REQUIRED_ENV_VARS.BASIC): { 
  success: boolean; 
  missing: string[];
} {
