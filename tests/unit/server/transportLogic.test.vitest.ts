...
describe("Transport Logic Tests", () => {
  describe("Transport Selection", () => {
    const selectTransport = (transportType: string | undefined) => {
      ...
      if (type === "sse" || type === "ws") {
        ...
        return { 
          selected: "streamable", 
          fallback: false,
          message: "HTTP transport - individual requests will create their own transports"
        };
    ...
    it("should fallback to stdio for streaming", () => {
      ...
      expect(result.selected).toBe("stdio");
      expect(result.fallback).toBe(true);
      expect(result.reason).toContain("Streaming transport not currently implemented");
    });
  });
  ...
  describe("Session Validation", () => {
    ...
    const shouldAllowRequest = (sessionId: string | undefined, body: any, sessions: Set<string>): boolean => {
    ...
    it("should reject invalid initialize requests", () => {
      ...
      expect(isInitializeRequest({ jsonrpc: "2.0", method: "tools/call" })).toBe(false);
    });
  ...
  describe("Environment Validation", () => {
    const validateRequiredEnvVars = (env: Record<string, string | undefined>): string[] => {
      const required = [
