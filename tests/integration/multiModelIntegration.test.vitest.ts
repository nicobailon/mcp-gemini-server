...
describe('Multi-Model Integration Tests', () => {
  ...
  describe('End-to-End Configuration Flow', () => {
    it('should properly configure and use multi-model setup', () => {
      process.env.GOOGLE_GEMINI_MODELS = '["gemini-2.5-pro-preview-05-06", "gemini-1.5-flash"]';
      process.env.GOOGLE_GEMINI_IMAGE_MODELS = '["imagen-3.0-generate-002"]';
      process.env.GOOGLE_GEMINI_CODE_MODELS = '["gemini-2.5-pro-preview-05-06"]';
      ...
      expect(modelConfig.textGeneration).toEqual(['gemini-2.5-pro-preview-05-06', 'gemini-1.5-flash']);
      ...
      expect(selectionService.isModelAvailable('gemini-2.5-pro-preview-05-06')).toBe(true);
      expect(selectionService.isModelAvailable('imagen-3.0-generate-002')).toBe(true);
    });
    ...
    it('should handle model selection for different task types', async () => {
      ...
      expect(['gemini-2.5-pro-preview-05-06', 'gemini-1.5-flash']).toContain(textModel);
  ...
  describe('Performance and Reliability', () => {
    it('should handle model selection performance metrics', async () => {
      ...
      selectionService.updatePerformanceMetrics('gemini-2.5-pro-preview-05-06', 2000, false);
      selectionService.updatePerformanceMetrics('gemini-2.5-pro-preview-05-06', 1800, false);
      selectionService.updatePerformanceMetrics('gemini-2.5-pro-preview-05-06', 2200, false);
      selectionService.updatePerformanceMetrics('gemini-2.5-pro-preview-05-06', 1900, false);
      selectionService.updatePerformanceMetrics('gemini-2.5-pro-preview-05-06', 2100, false);
    ...
    it('should maintain selection history', async () => {
      ...
      await selectionService.selectOptimalModel({ taskType: 'text-generation' });
  ...
  describe('Error Handling and Edge Cases', () => {
    ...
    it('should handle empty model arrays', () => {
      ...
      expect(modelConfig.textGeneration).toEqual(['gemini-1.5-flash-latest']);
    });
  });
  ...
  describe('Configuration Validation', () => {
    it('should validate model capabilities consistency', () => {
      ...
      Object.entries(modelConfig.capabilities).forEach(([modelName, capabilities]) => {
        ...
        expect(['none', 'basic', 'good', 'excellent']).toContain(capabilities.codeExecution);
        expect(['none', 'basic', 'good', 'excellent']).toContain(capabilities.complexReasoning);
