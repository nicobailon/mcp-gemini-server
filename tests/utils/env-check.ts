...
import { loadTestEnv, verifyEnvVars, REQUIRED_ENV_VARS, createEnvExample } from './environment.js';
 ...
 */
export function preCheckEnv(requiredVars: string[] = REQUIRED_ENV_VARS.BASIC): boolean {
  const check = verifyEnvVars(requiredVars);
  
  if (!check.success) {
    console.warn(`⚠️  Skipping tests - missing required environment variables: ${check.missing.join(', ')}`);
    return false;
  }
 ...
 */
export function skipIfEnvMissing(t: any, requiredVars: string[] = REQUIRED_ENV_VARS.BASIC): boolean {
  const check = verifyEnvVars(requiredVars);
