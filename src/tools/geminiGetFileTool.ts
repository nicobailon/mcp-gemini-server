 ...
 */
export const geminiGetFileTool = (server: McpServer, geminiService: GeminiService): void => {

    // Define the tool processing function
    const processRequest = async (params: GetFileParams): Promise<{ content: { type: 'text', text: string }[] }> => {
        logger.info(`Processing ${TOOL_NAME_GET_FILE} request for file: ${params.fileName}`);
        logger.debug("Received params:", params);

        try {
            // Call the GeminiService method
            const fileMetadata: FileMetadata = await geminiService.getFile(params.fileName);

            logger.info(`Successfully retrieved metadata for file: ${fileMetadata.name}`);
            ...
            logger.error(`Error processing ${TOOL_NAME_GET_FILE} for file ${params.fileName}:`, error);
            ...
            if (error instanceof GeminiApiError) {
                ...
                throw new McpError(ErrorCode.InternalError, 'An unknown error occurred while getting file metadata.');
            }
        }
    };
