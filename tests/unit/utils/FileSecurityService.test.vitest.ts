...
describe("FileSecurityService", () => {
  ...
  describe("Constructor and Configuration", () => {
    ...
    it("should throw error when setting empty allowed directories", () => {
      ...
      expect(() => service.setAllowedDirectories([]))
    ...
    it("should throw error when setting non-absolute allowed directories", () => {
      ...
      expect(() => service.setAllowedDirectories(["./relative/path"]))
        .toThrow(ValidationError);
      expect(() => service.setAllowedDirectories(["./relative/path"]))
        .toThrow(/Directory path must be absolute/);
    });
    ...
    it("should throw error when setting non-absolute secure base path", () => {
      ...
      expect(() => service.setSecureBasePath("./relative/path"))
        .toThrow(ValidationError);
      expect(() => service.setSecureBasePath("./relative/path"))
  ...
  describe("Path Validation", () => {
    ...
    it("should validate paths with relative components", () => {
      const complexPath = path.join(TEST_DIR, ".", "subdir", "..", "test-file.txt");
    ...
    it("should reject paths outside allowed directories", () => {
      ...
      expect(() => service.validateAndResolvePath(outsidePath))
        .toThrow(ValidationError);
      expect(() => service.validateAndResolvePath(outsidePath))
    ...
    it("should reject paths with directory traversal", () => {
      const traversalPath = path.join(TEST_DIR, "..", "outside", "test-file.txt");
      
      expect(() => service.validateAndResolvePath(traversalPath))
        .toThrow(ValidationError);
      expect(() => service.validateAndResolvePath(traversalPath))
        .toThrow(/Access denied/);
    });
    ...
    it("should check file existence with mustExist option", () => {
      ...
      expect(() => service.validateAndResolvePath(nonExistentPath, { mustExist: true }))
        .toThrow(ValidationError);
      expect(() => service.validateAndResolvePath(nonExistentPath, { mustExist: true }))
  ...
  describe("isPathWithinAllowedDirs", () => {
    ...
    it("should use custom allowed directories when provided", () => {
      ...
      expect(service.isPathWithinAllowedDirs(outsidePath, [OUTSIDE_DIR])).toBe(true);
    });
  ...
  describe("fullyResolvePath", () => {
    ...
    it("should handle non-existent paths", async () => {
      const nonExistentPath = path.join(TEST_DIR, "non-existent", "test-file.txt");
    ...
    it("should reject symlinks pointing outside allowed directories", async () => {
      ...
      await expect(restrictedService.fullyResolvePath(symlinkPath))
    ...
    it("should reject symlinked parent directories pointing outside allowed directories", async () => {
      ...
      await expect(restrictedService.fullyResolvePath(filePath))
        .rejects.toThrow(ValidationError);
      await expect(restrictedService.fullyResolvePath(filePath))
        .rejects.toThrow(/Security error/);
    });
  ...
  describe("secureWriteFile", () => {
    ...
    it("should create directories if they don't exist", async () => {
      const nestedFilePath = path.join(TEST_DIR, "nested", "deep", "test-file.txt");
    ...
    it("should reject writing outside allowed directories", async () => {
      ...
      await expect(service.secureWriteFile(outsidePath, TEST_CONTENT))
        .rejects.toThrow(ValidationError);
      await expect(service.secureWriteFile(outsidePath, TEST_CONTENT))
    ...
    it("should reject overwriting existing files by default", async () => {
      ...
      await expect(service.secureWriteFile(filePath, TEST_CONTENT))
        .rejects.toThrow(ValidationError);
      await expect(service.secureWriteFile(filePath, TEST_CONTENT))
    ...
    it("should allow overwriting existing files with overwrite flag", async () => {
      ...
      await service.secureWriteFile(filePath, TEST_CONTENT, { overwrite: true });
