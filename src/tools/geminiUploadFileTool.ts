 ...
 */
export const geminiUploadFileTool = (server: McpServer, geminiService: GeminiService): void => {

    // Define the tool processing function
    const processRequest = async (params: UploadFileParams): Promise<{ content: { type: 'text', text: string }[] }> => {
        ...
        try {
            ...
            const fileMetadata: FileMetadata = await geminiService.uploadFile(
                params.filePath,
                Object.keys(fileMetadataParams).length > 0 ? fileMetadataParams : undefined
            );
            ...
            if (error instanceof GeminiApiError) {
                ...
                if (error.message.includes('ENOENT') || error.message.toLowerCase().includes('file not found')) { // Example check
                    throw new McpError(ErrorCode.InvalidParams, `Invalid file path: ${params.filePath}`, error.details);
                }
                ...
                throw new McpError(ErrorCode.InternalError, 'An unknown error occurred during file upload.');
            }
        }
    };
